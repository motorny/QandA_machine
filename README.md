...
При сборке юнит-тестов необходимо указать корректную версию QT, установленную на компьютере пользователя:
На панели Solution Explorer необходимо вызвать контекстное меню у Solution и выбрать пункт "Change solution's Qt version", также в зависимости от установленного плагина, может потребоваться вызвать контекстное меню у пректа "tests" и, обратившись к пунтку "Qt project settings", указать в появившемся окне корректную версию Qt.
